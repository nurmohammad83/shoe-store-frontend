...
const CartItem = ({data}) => {
  const {name,price,subtitle,thumbnail,size} = data?.attributes
  ...
  return (
    <div className="flex py-5 gap-3 md:gap-5 border-b">
      ...
      <div className="shrink-0 aspect-square w-[50px] md:w-[120px]">
        <Image src={thumbnail.data.attributes.url} alt={name} width={120} height={120} />
      </div>
      {/* IMAGE END */}
          ...
          <div className="flex items-center gap-2 md:gap-10 text-black/[0.5] text-sm md:text-md">
            ...
            <div className="flex items-center gap-1">
              ...
              <select className="hover:text-black" onChange={(e)=>updateCartItem(e,'quantity')}>
                {
                  Array.from({length : 10}, (_1, i)=> i + 1).map((q,i)=>(
